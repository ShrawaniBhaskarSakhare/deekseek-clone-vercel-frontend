...
function Promt() {
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      if (storedPromt) setPromt(JSON.parse(storedPromt));
    }
  }, []);

  useEffect(() => {
    const user = JSON.parse(localStorage.getItem("user"));
    if (user) localStorage.setItem(`promtHistory_${user._id}`, JSON.stringify(promt));
  }, [promt]);
    ...
    <div className="flex flex-col items-center justify-between flex-1 w-full px-4 pb-4 md:pb-8">
      <div className="mt-8 md:mt-16 text-center">
        <div className="flex items-center justify-center gap-2">
          ...
          <h1 className="text-2xl md:text-3xl font-semibold text-white mb-2">Hi, I'm DeepSeek.</h1>
        </div>
        <p className="text-gray-400 text-base md:text-sm mt-2">💬 How can I help you today?</p>
      </div>
